...
| 触发时机 | 通知标题 | 通知内容 |
|---------|---------|---------|
| 确认（t=0 失焦） | cc-ribbit 🐸 | CC 在等你确认 🐸 |
| 确认（t=30s 失焦） | cc-ribbit 🐸 | CC 还在等你... 🐸🐸🐸 |
| 确认（t=60s 失焦） | cc-ribbit 🐸 | CC 派了增援！整个池塘都来了 🐸🐸🐸🐸🐸 |
| 任务完成（失焦） | cc-ribbit 🐸 | 干完了，青蛙复命 🐸 |
| 报错（失焦） | cc-ribbit 🐸 | 出事了，但还是可爱地告诉你 🐱 |
...
### v1.1（待实现）
- [ ] 焦点状态检测（macOS osascript + Linux xdotool）
- [ ] 任务耗时读取（从 hook stdin 读 `duration_ms`）
- [ ] 3 秒礼貌延迟（焦点内 PermissionRequest 等 3s 再呱）
- [ ] 焦点内短任务静默（Stop 耗时 < 30s 不响）
- [ ] 焦点内不弹系统通知（通知只在失焦时推送）
- [ ] 定时器焦点状态实时判断（t=30s/60s 触发时现查焦点）
